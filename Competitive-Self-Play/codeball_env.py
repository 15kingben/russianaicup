...
# import six
import sys
sys.path.append("/home/ben/russianaicup/Competitive-Self-Play/codeball-runner/python3-cgdk/")
from codeball_runner import CodeballRunner
from codeball_agent import CodeballAgent

class CodeballEnvVanilla(Env):
    '''
    ...
    def __init__(self, index, gui = True, nostart=False):
        '''
        ...
        self.agents = {}
        if not nostart:
            self.codeball_runner = CodeballRunner(index, gui)
        # all_agent_xml_paths = []
        # if not agent_args:
        ...
        for i in range(self.n_agents):
            self.agents[i] = CodeballAgent(i, 2)
    ...
    def step(self, action1, action2):
        # returns ob, reward, episode_over, info
        ...
        self.codeball_runner.simulate((action1, action2))
        reward1, reward2 = self.codeball_runner.get_reward()
        rewards = tuple([reward1, reward2])
        done = self.codeball_runner.get_done()
        dones = tuple([done, done])
        ob1, ob2 = self.codeball_runner.get_obs()
        ob1 = np.array(ob1)
        ob2 = np.array(ob2)
        # return obs, rewards, dones, {}
        return ob1, rewards[0], dones[0], ob2, rewards[1], dones[1]

    def reset(self):
        ob1, ob2 = self.codeball_runner.reset()
        # obs = tuple(ob + p2ob)
        return np.array(ob1), np.array(ob2)
