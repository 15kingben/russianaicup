...
import numpy as np
from codeball_constants import *

class CodeballAgent(object):
    '''
    ...
    def __init__(self, agent_id, nagents=2):
        self.id = agent_id
    ...
    def set_observation_space(self):
        ...
        #   positoin: (x,y,z)
        #   velocity: (x,y,z)
        standard_pos_low  = [-1*ARENA_WIDTH/2, 0, -1*ARENA_DEPTH/2 - ARENA_GOAL_DEPTH]
        standard_pos_high = [ARENA_WIDTH/2, ARENA_HEIGHT, ARENA_DEPTH/2 + ARENA_GOAL_DEPTH]

        standard_vel_low = [-1*MAX_ENTITY_SPEED, -1*MAX_ENTITY_SPEED, -1*MAX_ENTITY_SPEED]
        standard_vel_high = [MAX_ENTITY_SPEED, MAX_ENTITY_SPEED, MAX_ENTITY_SPEED]

        low = standard_pos_low + standard_vel_low
        high = standard_pos_high + standard_vel_high
        # Ball + 4 players
        low *= 5
        high *= 5
    ...
    def set_action_space(self):
        ...
        # ignoring for now: #     use_nitro: Bool // Whether nitro should be used

        # action space for both robots on team
        low = np.array([-1*ROBOT_MAX_GROUND_SPEED, -1*ROBOT_MAX_GROUND_SPEED, -1*ROBOT_MAX_GROUND_SPEED, 0,
                        -1*ROBOT_MAX_GROUND_SPEED, -1*ROBOT_MAX_GROUND_SPEED, -1*ROBOT_MAX_GROUND_SPEED, 0])
        high = np.array([ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_JUMP_SPEED,
                         ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_GROUND_SPEED, ROBOT_MAX_JUMP_SPEED])
        return Box(low, high)
